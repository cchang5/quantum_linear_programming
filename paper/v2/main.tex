...
\begin{document}
\title[QA and Open Quantum Systems for MDS]{A Study of Quantum Annealing and Open Quantum Systems for the Minimum Dominating Set}
...
\begin{abstract}
 ...
 For this reason, we developed an algorithm that solves integer linear programming problems and its application to the minimum dominating set, a classically NP-hard problem, on a quantum annealer, and investigated problem and hardware-specific limitations.
...
For general graphs, existing algorithms on classical computers find MDS solutions in exponential time $\sim O( 1.5^n)$ \cite{Fomin2009, vanRooij2009} or feasible solutions in polynomial time. For example, greedy algorithms locally optimize decisions about which nodes to add to the dominating set.
...
\end{equation}
and $H^\textrm{init}=-\sum_i\sigma^x_i$ (on the D-Wave), while $H^\textrm{problem}$ encodes the problem to be solved. In practice, modeling dynamics that arise during quantum annealing requires a more robust description of the thermally populated mixed quantum states and the open dynamical processes that govern population of the sought-after eigenstate.
...
In this paper, we present the mapping of ILPs to QUBOs and solve specific realizations, MDS problems for specific graphs, on a quantum annealer.
...
In Sec.~\ref{sec:methods}, we display how a general ILP is mapped onto QUBOs (Sec.~\ref{sec:results:qa1}), provide an explicit example in case of the MDS problem (Sec.~\ref{sec:methods:mds-qubo}), specify how the QUBO is embedded on the annealer (Sec.~\ref{sec:methods:MDS-on-D-Wave}), how the annealing schedules are realized (Sec.~\ref{sec:methods:anneal-schedule}), and how we have simulated the annealer (Sec.~\ref{sec:methods:simulation}). In Sec.~\ref{sec:results} we present our results and discuss them in Sec.~\ref{sec:discussion}. All data and software utilized in this paper can be found online (see also Sec.~\ref{sec:data}).
...
%==========================================
\subsection{MDS on the D-Wave}
\label{sec:methods:MDS-on-D-Wave}
...
We demonstrate the proposed algorithm in order to obtain the MDS on a series of linear graphs $G(n)$, as shown in Fig.~\ref{fig:linear}. This type of graph is chosen because the small number of nearest-neighbor connections is more efficiently embedded into the chimera graph, allowing scaling plots to be generated when using a D-Wave quantum annealer. In particular, the number of qubits required to solve MDS scales at worse as $n_V \log_2 n_V$ where $n_V$ is the number of vertices for a generic graph before minor embedding.
...
For even the smallest graph $G(2)$, the MDS problem is not native to the chimera graph and must be embedded. Following the hypothesis of MBL, we, therefore, must look at the values of $h_i$ after embedding. The qubits split into two groups depending on the value of $h_i$ relative to $(\textrm{max}|\{h\}| + \textrm{min}|\{h\}|) / 2$ given the set of external magnetic fields $\{h\}$ defined by a specific embedding.
...
Following the MBL inspired offset strategy, qubit(s) 0 (1, 2, 3, 4) are placed in the set with relatively stronger (weaker) final external fields.
...
\label{sec:methods:anneal-schedule}
%----------------------------------------------------------------------------------------

Our proposed offset strategy is motivated by the structure of the Hamiltonian being evaluated by the annealer.
On D-Wave solvers, annealing offsets effectively advance or delay the annealing schedule of individual qubits.
...
\begin{figure}
	...
	\caption{Time-dependent probability for resolving the ground state for simulation results presented in Sec.~\ref{sec:methods:simulation}. Different offsets are labeled in the same way as Fig.~\ref{fig:baseline}.}
	\label{fig:td_prob}
\end{figure}
