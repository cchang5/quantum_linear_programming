...
where
\begin{align}
Q_{00} = & {T^{x}}^T \left[ d + p A^T A + p \mathrm{Diag}_{x} \left(A^T b + b^T A\right) \right] T^x + \mathrm{Diag}_{\psi^x}(c) T^x,\\
Q_{01} = & Q_{10}^T = p {T^{x}}^T A^T T^s,\\
Q_{11} = & p\left[ {T^{s}}^T T^s + \mathrm{Diag}_{\psi^s}\left( {T^{s}}^T b + b^T T^s\right) \right].
\end{align}
