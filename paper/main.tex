...
\begin{align}
 \label{eq:ilp-constraints}
  ...
  & x_i  \in \mathbbm{N}\, ,
\end{align}
where the natural numbers $\mathbbm{N}$ include zero, $i=1, \cdots,  N$ is the number of dependent variables and $a=1, \cdots, M$ the number of constraint equations. {\color{red} I don't actually understand this sentence.}
...
\begin{align}
  ...
  & x_i \in \mathbb{N}.
\end{align}
As a result, we can restrict both $x_i$ and $s_i$ to sample over only positive integer values. {\color{blue} In general, $s$ can be relaxed to span a fixed-point decimal number following Ref.~\cite{Chang:2018uoc} if the constraint equation is defined by non-integer coefficients.}
...
If the annealer does not reach thermal equilibrium, for example, strong external magnetic fields freeze in individual qubits, the final solution of the annealing process might not encode the ground state of the final Hamiltonian. {\color{red} @CC is this statement actually correct?}
...
A solution to the ILP is found by obtaining the qubit vector $\psi$ which minimizes $E$.
The integer solution to the original ILP is computed by $\vec x = T^x \vec \psi^x$ and the original energy is computed by shifting the annealer extracted energy by $pb^2$.
The slack components of this vector $\psi^s$ can be utilized to check if the contraints are indeed fulfilled.
...
%----------------------------------------------------------------------------------------
{\color{red} I think we discussed enough about this in the main text. There's no need to talk about this here.}
