...
The results in yellow (green) delays the annealing of qubits subject to stronger (weaker) external fields. We observe improvement (diminishment) in the experimental results when qubits subject to stronger (weaker) final external fields are delayed in the anneal schedule.
...
An additional study where an extended annealing schedule is employed in the simulation which removes systematic errors introduced by annealing offsets and effects of local damping are presented in Sec.~\ref{sec:methods:annealing-schedule}.
...
We have presented a general mapping for ILP problems to QUBO representations used by Quantum Annelears.
We have tested this mapping's scaling in case of the MDS problem for line graphs $G(v)$.
While the mapping outperforms random guessing for finding the optimal solution to the problem, the application of this problem in QA, for current hardware, is currently only successful for relatively small graphs.
The applicability range depends more on the finite precision of matching the QUBO coefficients to hardware constraints and less from the qubit scaling itself.

As this mapping comes with a tunable parameter, such constraints can be lifted in a problem-specific manner to extend the applicability (Sec.~\ref{sec:methods:ilp-qubo-comments}).
Furthermore, the same algorithm can be extended to quadratic constraint optimization problems (Sec.~\ref{sec:methods:ilp:quadratic}).

We have also proposed a new annealing optimization scheme based on the MBL hypothesis--introducing offsets for delaying the anneal schedules of individual qubits based on the external magnetic field--which generally improves the probability of finding correct ground states of the system.
This phenomenon is observed across different problem sizes and hints at the possibility of a generic improvement strategy.

To further understand the nature of MBL inspired anneal schedule offset approach's origin, we simulate the Annealer for an MDS Problem G(2) graph.
This simulation utilizes two decoherence models, local dampening and full-counting statistics, with three free parameters.
We find that Annealing offsets lift the problems degeneracy and non-trivially, resulting in different, offset-based final state distributions.
...
For the simulation to reproduce the experiments final state distribution, effects of full-counting statistics are required.
For fixed simulation parameters, scaling concerning offset from the simulation matches what is observed from the experiment, which provides evidence suggesting that the improvements are related to quantum mechanics.

Analyzing the effects of both the decoherence model individually (Sec.~\ref{sec:methods:annealing-schedule}), we observe in both cases that the overall scaling follows the story of the MBL hypothesis.
Perhaps more importantly, we observe that a simulation without local decoherence, where the relaxation is dependent on precisely the instantaneous value of the external magnetic field, exhibits the same scaling as the experiment.
This result suggests that our strategy for setting annealing offsets is improving the algorithm beyond the simple explanation of qubits freezing due to single-particle amplitude damping.
The simulation with only amplitude damping does not fully capture the results of the experiment.
We observe hints of a phase transition concerning offset (which may be considered some measure of disorder) when dynamical thermalization effects are removed.
This observation is a tantalizing and strong evidence for the inclusion of the full-counting statistics model.
...
\begin{itemize}
    \item[\texttt{qlp}] for mapping MDS problems to QUBOs and perfoming simulations,
...
We provide further documentation in the repository readme file.
...
Ostrowski guided the ILP problem selection and Humble guided D-Wave related efforts.
Calculations on the D-Wave were performed  by Chang and K\"orber.
