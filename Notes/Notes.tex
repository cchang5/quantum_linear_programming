...
We want to find a minimum dominating set $D_\text{min}$ for a given graph $G(V,E)$.
A \textit{dominating~set} $D$ is a set of vertices $V$ such that all vertices in the graph which are not in $D$ are adjacent to at least one point in this set.
In other words each vertex in this graph is either in the dominating set or in the neighborhood $\mathcal N_i$ of a vertex $i \in D$.
When $x_i = 0, 1$ is a variable representing if vertex $i$ is in the dominating set ($x_i = 1$), this constrained can be written as $|V|$ (number of all vertices) inequalities
\begin{equation}\label{eq:ms-constraint}
	x_i + \sum_{j \in \mathcal N_i} x_j \geq 1 \, , \qquad x_i = 0, 1 \, , \qquad \forall_{i \in V} \, .
\end{equation}

A \textit{minimal~dominating~set} is a dominating set which cannot be reduced by taking out vertices---if you are able to remove a vertex of your dominating set and still have a dominating set, it was not minimal.

Finally, a \textit{minimum~dominating~set} (note that \textbf{minimum} $\neq$  \textbf{minimal}) is the smallest minimal dominating set you can find.
...
Thus, the integer linear programming representation of the problem is given by eq.~\eqref{eq:ms-constraint} and
\begin{equation}\label{mds:min-con-classical}
	\min_{i \in V} \sum_i x_i \, .
\end{equation}
...
\end{equation}
The solution to the Minimum Dominating Set Problem (MDSP) is found by finding the optimal vector $\vec \psi = (\vec x, \vec s)^T$ such that
...
\end{equation}
is minimal.
To guarantee that the global minimum solution causes the constraint to be zero, $p$ needs to be larger than the minimization condition eq.~\eqref{mds:min-con-classical}.
In principal, $p = |V| + 1$ is generally sufficient, it might be that one has to optimize this parameter in order to simplify convergence on the D-Wave.
...
For a given slack variable $s_i$, we find the smallest integer $b_i$ such that $2^{b_i}$ is larger or equal to the number of neighbors of vertex $i$
\begin{equation}
	...
	\, .
\end{equation}
For this choice, the basis of the QUBO $Q$ can be written by
\begin{align}\label{mds:qubo}
	...
	\, . 
\end{align}
for the bit vector $\vec q$ which relates to $\vec s$ using the bitmap $\vec B$
\begin{equation}
...
The derivation of the QUBO utilizes that components of the bit vector squared are equal to the linear component, $\psi_\nu^2 = \psi_\nu$, and thus the QUBO represents the original objective function $f$ modulo a known constant.
The block matrices can be found by multiplying out eq.~\eqref{mds:objective-function} and identifying the linear and quadratic components
\begin{align}
...
\end{align}
Given the adjacency matrix $A$ (symmetric matrix representing the graph with and zero diagonal and entries being either one or zero), one finds
\begin{align}
	\Omega &= \mathbbm{1} \, ,
    &
...
The QUBO $Q$ is block upper-triangular, $\gamma$ is block diagonal and $\alpha, \beta$ are relatively dense because of the neighbors-to-neighbors entries ($A^2$) and neighbors-to-slack-bits entries ($A B$).
...
Next, the module classically searches the full bit-space for small example graphs to verify the solutions returned by the algorithm make sense.
Below you can find a few example plots.

If you run `\texttt{python qlp/mds/qubo.py}'\footnote{You have to install the scripts by running `\texttt{python~-m~pip~install~.}' before.} from the repo root, you will generate a random graph, compute the minimum QUBO, find the global minimum solutions and visualize them.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{mds-sol-n5-s.png}
	\caption{
		\label{fig:mds-sol-1}
	}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.6\textwidth]{mds-sol-n5.png}
	\caption{
		\label{fig:mds-sol-2}
	}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{mds-sol-n6.png}
	\caption{
		\label{fig:mds-sol-3}
	}
\end{figure}
