...
\begin{align}
P =\sum_i | \langle \psi (t)  | \mbox{gnd}_i \rangle |^2 ,
\end{align}
where the summation runs over degenerated ground states.
...
\end{align}
where the projection operator onto the ground state subspace is $\pi_{\mbox{gnd}}=\sum_i |\mbox{gnd}_i\rangle \langle \mbox{gnd}_i| $.
...
We allow the ability to define arbitrary annealing schedules as shown in Fig.~\ref{fig:anneal_schedules}. We also allow for per qubit offsets as suggested by Eq.~(\ref{eq:annealH}), with an example annealing schedule shown in Fig.~\ref{fig:anneal_offset} as well. For DWave's implementation, anneal offset yields systematic errors because the qubits no longer all start and end with the same coefficients. For our simulation, we allow for extended anneal schedules, which allow all qubits to undergo the full annealing process, yielding a true temporal offset for inhomogeneous driving fields.
