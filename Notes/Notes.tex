...
\begin{align}
H_{anneal}(t) =  \frac{-A(t)}{2} (\sum_i \sigma^x_i) + \frac{B(t)}{2} (\sum_{i>j} J_{ij} \sigma^z_i \sigma^z_j +\sum_i h_i \sigma^z_i ) 
\end{align}
...
\begin{align}
\psi(t=0)= \otimes_i  |\rightarrow \rangle_i
\end{align}
...
For now the offset is included in the following way: 
\begin{align}
H_{offset}(t) =  \frac{-A(t)}{2} ( \sum_i c_i(t) \sigma^x_i) + \frac{B(t)}{2} (\sum_{i>j} \frac{c_i(t)+c_j(t)}{2} \sigma^z_i \sigma^z_j +\sum_i  c_i(t) \sigma^z_i ) 
\end{align}


So the total Hamiltonian is $H=H_{anneal}+H_{offset}$.

Users can define functions $c_i(t)$, $A(t)$, $B(t)$ in the code.
...
\begin{figure}[htb]
	...
	\caption{
		\label{tdse} time-dependent Schrodinger equation simulation of quantum annealing.
	}
\end{figure}
