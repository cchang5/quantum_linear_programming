...
In the bit basis, $f$ is implemented as
\begin{equation}
	M_{\alpha \beta}
        =
...
\begin{figure}
	...
	\caption{
		...
		The green dots display the location of the maximal value under the constrained are obtained by running the \codeword{qlp} toy problem.
		Thus (A) does not change the location of the maximum, (B) shifts it and (C) shifts it but is degenerate.
	}
\end{figure}
...
Thus the vector-matrix-vector product can be negative and thus result in negative constraint values (increasing the total optimal maximization value).
...
The full matrix adding including the constraints is obtained by running \codeword{qlp.example.get_omega}.
...
The example is run in the notebook \codeword{notebooks/example-problem.ipynb}.
